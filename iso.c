...
u8 GetFileInfo(char *iso, file_info_t **file_info, u32 *file_number)
{
    ...
    *file_number = 0;
        ...
        while(1) {
            ...
            while(1) {
                ...
                if((int) idr->name_len[0] > 1 && idr->flags[0] != 0x2 &&
					...
					local = (file_info_t *) realloc(local, (*file_number + 1) * sizeof(file_info_t));
                    local[*file_number].sector = file_lba;
                    local[*file_number].size = file_size; 
                    local[*file_number].path = strcpy_malloc(string2);
    ...
    // sort
    *file_info = (file_info_t *) malloc((*file_number) * sizeof(file_info_t) );
	
    u32 i,j;
	u32 smallest_sector = 0;
	for(j=0; j<(*file_number); j++) {
		
		for(i=0; i<(*file_number); i++) {
			if( local[i].sector < local[smallest_sector].sector ) {
				smallest_sector = i;
			}
		}
		(*file_info)[j].size = local[smallest_sector].size;
		(*file_info)[j].sector = local[smallest_sector].sector;
		(*file_info)[j].path = strcpy_malloc(local[smallest_sector].path);
		
		local[smallest_sector].sector = -1; // MAX !
	}
	
	for(j=0; j<(*file_number); j++) FREE(local[j].path);
	FREE(local);
